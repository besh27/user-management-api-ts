...
import dbBilling from '../../db/dbBilling';
import { Router, Request, Response } from 'express';

const bills = dbBilling;
...
router.get('/', async (req: Request, res: Response) => {
    ...
    try {
        if (!bills) res.status(404).send('This resource is unavailable.');
...
router.get('/:id', (req, res) => {
    const bill = bills.find(c => c.id === parseInt(req.params.id));

    if (!bill) res.status(404).send('This user id was not found in this resouce.');
